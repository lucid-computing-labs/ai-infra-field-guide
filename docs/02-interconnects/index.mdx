...
```mermaid
flowchart TB
    subgraph NVLinkDomain["NVLink Domain (INVISIBLE to network monitoring)"]
        direction LR
    ...
    subgraph IBFabric["InfiniBand Fabric (OBSERVABLE)"]
        direction LR
        LS1["Leaf Switch"] <-->|"IB NDR\n400 Gb/s"| SS1["Spine Switch"]
        LS2["Leaf Switch"] <-->|"IB NDR\n400 Gb/s"| SS1
    end
    NVLinkDomain -->|"ConnectX NIC"| LS1
    style NVLinkDomain fill:#1a237e,color:#fff
    style IBFabric fill:#1b5e20,color:#fff
```
...
<KeyNumberRow>
  <KeyNumber value="900 GB/s" label="NVLink Bandwidth" context="Bidirectional per GPU" />
  <KeyNumber value="1-2 \u00B5s" label="NVLink Latency" context="Sub-microsecond" />
  <KeyNumber value="72 GPUs" label="Domain Size" context="NVL72 all-to-all via NVSwitch" />
</KeyNumberRow>
...
<KeyNumberRow>
  <KeyNumber value="18:1" label="Bandwidth Ratio" context="NVLink (900 GB/s) vs IB (50 GB/s)" />
</KeyNumberRow>
